...
Internal link: [[About%20Macros][Link to a section]].
...
This is a 3-level nested list:
  ...
  - Item one.
    ...
    - Term TWO :: This is term two.
                  ...
                  2. Description two.
                     #+BEGIN_EXAMPLE
                     Code for "Description two" (line 1)
                     Code for "Description two" (line 2)
                     #+END_EXAMPLE
                  3. Description three.

                     The following example is part of Item "Description
                     three":
                     : Code line 1 for "Description three".
                     : Code line 2 for "Description three".

                  The following example /breaks/ the above ordered list
                  because of the indentation level of =#+BEGIN_EXAMPLE=.
                  So this example becomes a paragraph that is parallel
                  to the above list.  Note that /all/ leading spaces in
                  the example are removed, which is not that perfect.

                  #+BEGIN_EXAMPLE
                            This is an example for Term TWO
                  #+END_EXAMPLE

                  After the above example, we now start a new ordered
                  list:

                  1. New list for Term TWO (item 1)
                  2. New list for Term TWO (item 2)
...
We can use macros to include current date and time in Org file, like
this: {{{date(%Y-%m-%d %H:%M:%S)}}}.  We can also use macros to expand
some commonly used abbreviations.  For example: "{{{TTL}}}" (expanded
from macro =TTL=) for cache can be configured in the following lines in
the configuration file:

#+BEGIN_EXAMPLE
{{{TTL}}} of cache: 1000 minutes.
#+END_EXAMPLE

* About =coderef=

For =coderef= like [[(foo)][FOOBIE]], DocBook exporter does not create any link as
the target is hard to define.  The description of the link is exported
as it is instead.
...
* About Labels and Cross References of Tables

If a label is specified for a table, we can use a cross reference to
refer to the table.  Below is an example:

#+LABEL: simple-table
#+CAPTION: A table with caption and label
| Col1 | Col2 |
|------+------|
| a    | 1    |
| b    | 2    |

Here is an reference of this table: [[simple-table][Link to the simple table]].

The following is a table without caption:

#+LABEL: simple-table-2
| Col1  | Col2 |
|------+------|
| aa   | 10   |
| bb   | 20   |
